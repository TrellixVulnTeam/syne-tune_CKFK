...
class Tuner:
    def __init__(
        ...
        start_jobs_without_delay: bool = False,
    ):
        """
        ...
        :param start_jobs_without_delay: By default, the tuner requests the number of
            busy workers from the backend before starting new ones. If this is True,
            it starts new jobs rather depending on scheduler decisions communicated
            to the backend. For example, if a trial has just been stopped (by calling
            `backend.stop_trial`), the tuner may start a new one immediately.
            This flag matters for backends where stopping or pausing trials is not
            immediate (e.g., :class:`SageMakerBackend`). If True, there will be more
            than `n_workers` jobs running at certain times, which precludes setting
            `n_workers` to the available quota for an instance type (doing so will
            lead to an exception). Also, if the SageMaker warm pool feature is used,
            there will be inefficiencies. On the other hand, setting this flag to
            True can lead to faster experiment runtime, because the backend is
